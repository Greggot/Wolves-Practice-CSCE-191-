...
#include "ui_prizmparametr.h"
#include "Engine.cpp"

PrizmParametr::PrizmParametr(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::PrizmParametr)
{
    ui->setupUi(this);
    this->setWindowTitle("Установка параметров призмы");
}

PrizmParametr::~PrizmParametr()
{
    delete ui;
}


void PrizmParametr::on_ok_clicked()
{
   ...
   sf::Color Figurecolor(0,0,255,255);
     ...
     Prism_Cylinder thing(vys, diag, tch*2, cent, Figurecolor);
     ...
     case 1:
     {
         ...
         Prism_Cylinder thing(vys,diag,tch*2,cent, shape);
         POINTS.push_back(thing.getPOINTS());
         POLYGONES.push_back(thing.getTriangle());
            break;
     }
...
void PrizmParametr::on_ok_2_clicked()
{
    int vys = (ui->vys->text()).toInt();
    int diag = (ui->dm->text()).toInt();
    int tch = (ui->rebro_5->text()).toInt();
